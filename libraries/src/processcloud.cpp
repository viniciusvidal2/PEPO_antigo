...
//                0.000000,    0.000000,   1.000000;
    ...
    K_cam << 2182.371971/2,    0.000000, 990,
...
//    float z = altura_eixo*sin(DEG2RAD(-tilt))*cos(DEG2RAD(-pan));
    // Vetor de centro
    Eigen::Vector3f t{0, 0, 0}; // Cinco centimetros e meio aprox. a frente nesse caso
    t = rot*t;       // Rotacionar pela orientacao dos servos
    T.block<3,1>(0, 3) = t;
    C = -rot.transpose()*t;
    // Preenche a mensagem de odometria do robo para mostrar no RViz
    msg.pose.pose.position.x = t(0); msg.pose.pose.position.y = t(1); msg.pose.pose.position.z = t(2);
