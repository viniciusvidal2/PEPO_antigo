...
void ProcessCloud::calculateNormals(PointCloud<PointTN>::Ptr acc_normal){
    ...
    ne.setSearchMethod(tree);
    if(acc_normal->size() > 50) ne.setKSearch(10); else ne.setKSearch(3);
    ne.setNumberOfThreads(20);
    ne.compute(*acc_normal);
//    cout << "COMPUTOU AS NORMAIS" << endl;
    removeNaNNormalsFromPointCloud<PointTN>(*acc_normal, *acc_normal, indicesnan);
...
//    cout << "Entrando no preprocesso com " << cin->size() << " pontos." << endl;
...
//    cout << "Removendo pontos nao projetados antes do processamento agora com " << cin->size() << " pontos." << endl;
    // Filtro de voxels para aliviar a entrada
    if(vs > 0){
        VoxelGrid<PointT> voxel;
        voxel.setLeafSize(vs, vs, vs);
//        cout << "Iniciando filtro de voxels e nuvem com " << cin->size() << " pontos.\n";
...
//    cout << "Iniciando filtro de depth e nuvem com " << cin->size() << " pontos.\n";
    pass.setInputCloud(cin);
    pass.filter(*cin);
    // Separando a nuvem de entrada em clusters
//    cout << "Separando em clusters a nuvem de entrada ..." << endl;
...
//                cout << "\n\nTamanho da nuvem grande no indice " << indice_grande << "  " << nuvem_grande->size() << endl;
                break;
            }
        }
...
//            cout << "Dividiu em " << big_clusters.size() << " e adicionando" << endl;
//            if(big_clusters.size() == 1)
//                cout << "NAO DIVIDIU CLUSTERS DE NOVO !!!" << endl;
//            cout << "tamanho da nuvem antes de substituir " << cin_clusters[indice_grande].size() << endl;
            cin_clusters[indice_grande] = big_clusters[0];
//            cout << "tamanho da nuvem que foi substituida  " << cin_clusters[indice_grande].size() << endl;
            cin_clusters.insert(cin_clusters.end(), big_clusters.begin()+1, big_clusters.end());
            big_clusters.clear(); nuvem_grande->clear();
//            cout << "Agora temos " << cin_clusters.size() << " clusters ao todo na divisao." << endl;
        }
    }
//    cout << "Clusters separados, total " << cin_clusters.size() << "." << endl;
...
#pragma omp parallel for
    for(size_t i=0; i<out_clusters.size(); i++){
        ...
        *temp = cin_clusters[i];
        ...
        // Filtro de vizinhos em um raio
//        RadiusOutlierRemoval<PointT> ror;
//        if(vs > 0) ror.setRadiusSearch(3*vs); else ror.setRadiusSearch(0.03);
//        if(vs > 0) ror.setMinNeighborsInRadius(4*int(100*vs)); else ror.setMinNeighborsInRadius(4);
        // Passando filtros
//        cout << "Iniciando filtro de raio e nuvem com " << temp->size() << " pontos.\n";
//        ror.setInputCloud(temp);
//        ror.filter(*temp);
//        cout << "Iniciando filtro estatistico e nuvem com " << temp->size() << " pontos.\n";
        sor.setInputCloud(temp);
        sor.filter(*temp);
        PointCloud<PointTN>::Ptr filt_normal (new PointCloud<PointTN>);
        if(fp == 1){
//            cout << "Iniciando filtro polinomial e nuvem com " << temp->size() << " pontos.\n";
...
std::string ProcessCloud::escreve_linha_sfm(string nome, Matrix3f r, Vector3f t){
    ...
    // Adicionar foco x e y como na matriz da camera em resolucao Full HD
    linha = linha + " " + std::to_string(K1(0, 0)) + " " + std::to_string(K1(1, 1));
    // Adicionar centro optico em x e y como na matriz da camera em resolucao Full HD
