...
//            sor.filter(*filtrada);
    ...
    // Acumula nuvem final evitando repeticao de pontos
    ROS_INFO("Acumulando clusters apos processo ...");
    *final += planos_totais[0];
    PointCloud<PointTN>::Ptr cnow (new PointCloud<PointTN>);
    for(size_t i=1; i < planos_totais.size(); i++){
        ROS_INFO("Somando plano %zu com %zu pontos ...", i+1, planos_totais[i].size());
        *cnow = planos_totais[i];
...
//        for(size_t i=0; i<cnow->size(); i++){
//            if(kdtree.radiusSearch(cnow->points[i], radius, pointIdxRadiusSearch, pointRadiusSquaredDistance) <= average_neighbors)
...
//        extract.filter(*cnow);
    ...
    for(size_t i=0; i < clusters_totais.size(); i++){
        ROS_INFO("Somando cluster %zu com %zu pontos ...", i+1, clusters_totais[i].size());
        *cnow = clusters_totais[i];
//        // Iniciar kdtree de busca
//        KdTreeFLANN<PointTN> kdtree;
//        kdtree.setInputCloud(final);
//        vector<int> pointIdxRadiusSearch;
...
//        extract.filter(*cnow);
    ...
    int retorno = system(("rm -r "+pasta_cluters).c_str());
